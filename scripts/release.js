...
const release = async () => {
  ...
  const lernaArgs = ['publish', version];
  ...
  const versionMarkerPath = path.resolve(
    __dirname,
    '../packages/cli-version-marker/package.json'
  );
  ...
  fs.writeFileSync(
    versionMarkerPath,
    JSON.stringify(versionMarkerPkg, null, 2)
  );

  const tagName = `cli-version-marker@${versionMarkerPkg.version}`;
  await execa('git', ['add', '-A'], { stdio: 'inherit' });
  await execa('git', ['commit', '-m', `chore: ${tagName}`], {
    stdio: 'inherit',
  });
  ...
  await execa('npm', ['publish', '--registry', 'https://registry.npmjs.org/'], {
    stdio: 'inherit',
    cwd: path.dirname(versionMarkerPath),
  });
